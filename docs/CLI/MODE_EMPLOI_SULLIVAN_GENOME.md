...
- **Sullivan** (côté API) : Analyse backend, inférence UI (UIInferenceEngine), génération de composants (ComponentGenerator) via **Aetherflow** (plans, PROTO/PROD). Voir `docs/PRD_SULLIVAN.md`.
...
- **CLI** : `./aetherflow -q --plan <plan.json>` (rapide) ou `./aetherflow -f --plan <plan.json>` (qualité). Voir `docs/GUIDE_RAPIDE_AETHERFLOW.md`.
...
- Clés API configurées (`.env`) pour les providers utilisés (Groq, DeepSeek, Gemini, etc.) selon `docs/INSTALLATION.md` ou `.env.example`.
...
- **Aetherflow (quick start)** : `docs/GUIDE_RAPIDE_AETHERFLOW.md`
- **Installation** : `docs/INSTALLATION.md`
- **Double-check chatbox** : `docs/DOUBLE_CHECK_CHATBOX_SULLIVAN.md`
