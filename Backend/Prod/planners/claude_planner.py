"""Claude Sonnet planner for generating execution plans."""
import json
import uuid
...
class ClaudePlanner:
    """Plan planner using Claude Sonnet 3.5."""
...
Generate the plan.json following the schema above. Return ONLY the JSON, no markdown formatting."""
    ...
    async def generate_plan(
        ...
        """
        logger.info(f"Claude Planner: Generating plan for '{description[:50]}...'")
        ...
        try:
            ...
            try:
                ...
                plan["metadata"]["claude_version"] = "claude-api"
    ...
    async def review_plan(
        ...
        """
        logger.info(f"Claude Planner: Reviewing plan {plan.get('task_id', 'unknown')}")
