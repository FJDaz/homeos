"""Gemini planner for generating execution plans."""
import json
import uuid
...
class GeminiPlanner:
    """Plan planner using Gemini 3 Pro."""
...
Generate the plan.json following the schema above. Return ONLY the JSON, no markdown formatting."""
    ...
    async def generate_plan(
        ...
        """
        logger.info(f"Gemini Planner: Generating plan for '{description[:50]}...'")
        
        # Build prompt
        system_prompt, user_prompt = self._build_planning_prompt(description, context)
        
        # Combine system and user prompt for Gemini (Gemini doesn't have separate system prompt)
    ...
    async def review_plan(
        ...
        """
        logger.info(f"Gemini Planner: Reviewing plan {plan.get('task_id', 'unknown')}")
