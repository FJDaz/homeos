...
class Settings(BaseSettings):
    ...
    deepseek_api_key: str = Field(
        default="", alias="DEEPSEEK_API_KEY", description="DeepSeek API key for code generation"
    )
    ...
    anthropic_api_key: str = Field(
        default="", alias="ANTHROPIC_API_KEY", description="Anthropic API key for Claude validation (automatic)"
    )

    hf_token: str = Field(default="", alias="HF_TOKEN", description="Hugging Face API token")

    kimi_api_key: str = Field(
        default="", alias="KIMI_KEY", description="KIMI (Moonshot) API key for gate-keeper validation"
    )
    ...
    deepseek_model: str = Field(default="deepseek-coder", alias="DEEPSEEK_MODEL", description="DeepSeek model to use")

    codestral_model: str = Field(
        default="codestral-latest", alias="CODESTRAL_MODEL", description="Codestral model to use"
    )
    ...
    # Request Parameters
    max_tokens: int = Field(default=4000, alias="MAX_TOKENS", description="Maximum tokens per request")

    temperature: float = Field(default=0.7, alias="TEMPERATURE", description="Temperature for API requests")
    ...
    max_retries: int = Field(default=3, alias="MAX_RETRIES", description="Maximum number of retry attempts")

    # Paths
    plan_file_path: str = Field(default="plan.json", alias="PLAN_FILE_PATH", description="Path to the plan JSON file")

    output_dir: Path = Field(default=Path("output"), alias="OUTPUT_DIR", description="Directory for output files")

    logs_dir: Path = Field(default=Path("logs"), alias="LOGS_DIR", description="Directory for log files")
    ...
    deepseek_input_cost_per_1k: float = Field(
        default=0.00014, alias="DEEPSEEK_INPUT_COST_PER_1K", description="Cost per 1K input tokens (USD)"
    )

    deepseek_output_cost_per_1k: float = Field(
        default=0.00028, alias="DEEPSEEK_OUTPUT_COST_PER_1K", description="Cost per 1K output tokens (USD)"
    )
    ...
    enable_claude_validation: bool = Field(
        default=False, alias="ENABLE_CLAUDE_VALIDATION", description="Enable Claude Code validation (manual, via files)"
    )
    ...
    groq_input_cost_per_1k: float = Field(
        default=0.00059, alias="GROQ_INPUT_COST_PER_1K", description="Cost per 1K input tokens for Groq (USD)"
    )
    ...
    max_concurrent_requests_deepseek: int = Field(
        default=5, alias="MAX_CONCURRENT_REQUESTS_DEEPSEEK", description="Maximum concurrent requests for DeepSeek"
    )

    max_concurrent_requests_groq: int = Field(
        default=10, alias="MAX_CONCURRENT_REQUESTS_GROQ", description="Maximum concurrent requests for Groq"
    )

    max_concurrent_requests_gemini: int = Field(
        default=10, alias="MAX_CONCURRENT_REQUESTS_GEMINI", description="Maximum concurrent requests for Gemini"
    )

    max_concurrent_requests_codestral: int = Field(
        default=5, alias="MAX_CONCURRENT_REQUESTS_CODESTRAL", description="Maximum concurrent requests for Codestral"
    )
