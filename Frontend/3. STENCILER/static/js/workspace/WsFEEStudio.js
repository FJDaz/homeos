...
class WsFEEStudio {
        ...
        overlay.innerHTML = `
            ...
            <header class="h-[48px] border-b border-[#e5e5e5] px-6 flex items-center justify-between bg-white shrink-0">
                <div class="flex items-center gap-4">
                    <span class="text-[12px] font-black tracking-[0.2em] uppercase text-[#3d3d3c]">fee studio <span class="text-[#8cc63f]">camera raw</span></span>
                    <div class="h-[18px] w-px bg-[#e5e5e5]"></div>
                    <span id="fee-studio-project-label" class="text-[12px] font-bold text-[#9a9a98] lowercase italic">Projet: ...</span>
                </div>
                <button id="fee-studio-btn-close" class="p-2 text-[#9a9a98] hover:text-[#3d3d3c] transition-colors rounded-[20px]">
                    <svg class="w-[22px] h-[22px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                </button>
            </header>
            ...
            <div class="flex-1 flex divide-x divide-[#e5e5e5] overflow-hidden">
                ...
                <aside class="w-[240px] flex flex-col bg-white shrink-0">
                    <div class="p-3 border-b border-[#e5e5e5] bg-[#f7f6f2]/50">
                        <span class="text-[11px] font-black uppercase tracking-widest text-[#9a9a98]">explorateur triggers</span>
                    </div>
                    <!-- State Selector -->
                    <div class="p-3 border-b border-[#e5e5e5] flex gap-1">
                        <select id="fee-studio-state" class="w-full text-[12px] font-bold border border-[#e5e5e5] bg-white px-2 py-1 outline-none text-lowercase rounded-[20px]">
                    ...
                    <div class="h-[48px] border-t border-[#e5e5e5] bg-white flex items-center justify-between px-4 shrink-0">
                        ...
                        <div class="flex items-center gap-2">
                            <button id="fee-studio-btn-zoom-out" class="w-7 h-7 rounded-full border border-[#e5e5e5] flex items-center justify-center text-[#9a9a98] hover:border-[#8cc63f] hover:text-[#8cc63f] transition-all" title="Zoom Out">
                                <svg class="w-[14px].5 h-[14px].5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>
                            </button>
                            <span id="fee-studio-zoom-level" class="text-[12px] font-bold text-[#9a9a98] w-[42px] text-center tabular-nums">100%</span>
                            <button id="fee-studio-btn-zoom-in" class="w-7 h-7 rounded-full border border-[#e5e5e5] flex items-center justify-center text-[#9a9a98] hover:border-[#8cc63f] hover:text-[#8cc63f] transition-all" title="Zoom In">
                                <svg class="w-[14px].5 h-[14px].5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                            </button>
                        </div>
                        <!-- Playback Controls -->
                        <div class="flex items-center gap-4">
                            <button id="fee-studio-btn-rewind" class="p-2 text-[#9a9a98] hover:text-[#8cc63f] transition-all rounded-[20px]" title="Rewind">
                                <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0019 16V8a1 1 0 00-1.6-.8l-5.334 4zM4.066 11.2a1 1 0 000 1.6l5.334 4A1 1 0 0011 16V8a1 1 0 00-1.6-.8l-5.334 4z"/></svg>
                            </button>
                            <button id="fee-studio-btn-play" class="w-[42px] h-[42px] rounded-full bg-[#3d3d3c] text-white flex items-center justify-center hover:bg-black transition-all shadow-lg active:scale-95">
                                <svg class="w-[18px] h-[18px] fill-current ml-0.5" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                            </button>
                            <button id="fee-studio-btn-slow" class="px-3 py-1 border border-[#e5e5e5] text-[11px] font-black uppercase tracking-widest text-[#9a9a98] hover:border-[#8cc63f] hover:text-[#8cc63f] transition-all rounded-[20px]" title="Slow Motion x0.25">
                                ×0.25
                            </button>
                        </div>
                        <!-- Download -->
                        <button id="fee-studio-btn-download" class="px-3 py-1.5 border border-[#e5e5e5] text-[11px] font-bold uppercase tracking-wider text-[#9a9a98] hover:border-[#8cc63f] hover:text-[#8cc63f] transition-all rounded-[20px]" title="Télécharger le projet">
                            <svg class="w-[14px].5 h-[14px].5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                            télécharger
                        </button>
                    </div>
                </main>
                ...
                <aside class="w-[320px] flex flex-col bg-[#f7f6f2] shrink-0">
                    <div class="p-3 border-b border-[#e5e5e5] flex items-center justify-between bg-white">
                        <span class="text-[11px] font-black uppercase tracking-widest">sullivan fee <span class="text-[#8cc63f]">worker</span></span>
                        <div id="fee-studio-status" class="w-[10px] h-[10px] rounded-full bg-[#ccc]"></div>
                    </div>
                    <!-- Chat History -->
                    <div id="fee-studio-chat" class="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar text-[13px] leading-relaxed">
                    ...
                    <div class="p-3 border-t border-[#e5e5e5] bg-white">
                        <textarea id="fee-studio-input" placeholder="Décrivez l'émotion visuelle..." class="w-full bg-[#f7f6f2] border border-[#e5e5e5] p-3 text-[14px] outline-none focus:border-[#8cc63f] h-20 resize-none rounded-[20px]"></textarea>
                        <div class="flex items-center justify-between mt-2">
                             <button id="fee-studio-btn-apply" class="px-3 py-1.5 bg-[#8cc63f] text-white text-[11px] font-black uppercase tracking-widest hidden rounded-[20px]">appliquer le code</button>
                             <button id="fee-studio-btn-send" class="ml-auto w-[34px] h-[34px] bg-[#3d3d3c] text-white flex items-center justify-center hover:bg-black transition-all rounded-[20px]">
            ...
            <footer class="h-[100px] border-t border-[#e5e5e5] bg-[#f7f6f2] overflow-hidden flex items-center shrink-0">
                <div class="px-4 border-r border-[#e5e5e5] h-full flex flex-col justify-center gap-1 shrink-0">
                    <span class="text-[10px] font-black uppercase tracking-tighter text-[#9a9a98]">pellicule</span>
                    <span class="text-[12px] font-bold">effets gsap</span>
                </div>
    ...
    async scanTriggers() {
        ...
        try {
            ...
            elements.forEach(el => {
                ...
                item.innerHTML = `
                    <div class="flex items-center gap-2">
                        <div class="w-[6px].5 h-[6px].5 rounded-full ${hasAnimation ? 'bg-[#8cc63f] shadow-[0_0_8px_rgba(140,198,63,0.6)]' : 'bg-[#efefef]'}" data-led-id="${id}"></div>
                        <span class="text-[13px] font-bold">${id}</span>
                    </div>
                    <span class="text-[10px] opacity-30 uppercase font-black">${tag}</span>
                `;
            ...
            data.presets.forEach(p => {
                ...
                card.innerHTML = `
                    <div class="flex-1 bg-[#f7f6f2] flex items-center justify-center mb-1 group-hover:bg-[#8cc63f]/10 rounded-[12px]">
                        <svg class="w-[26px] h-[26px] text-[#ccc] group-hover:text-[#8cc63f]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    </div>
                    <span class="text-[10px] font-black uppercase text-center truncate">${p.name}</span>
                `;
                card.onclick = () => {
    ...
    addChatMessage(role, text) {
        ...
        div.innerHTML = `<div class="text-[13px]">${text}</div>`;
    ...
    async downloadProject() {
        try {
            ...
            const btnDownload = document.getElementById('fee-studio-btn-download');
            if (btnDownload) btnDownload.innerHTML = '<svg class="w-[14px].5 h-[14px].5 inline mr-1 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>zipping...';
            ...
            if (btnDownload) btnDownload.innerHTML = '<svg class="w-[14px].5 h-[14px].5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>télécharger';
