...
(function() {
    ...
    function renderCritique(questions, suggestions) {
        ...
        questions.forEach(q => {
            ...
            card.innerHTML = `
                <span class="text-[11px] text-slate-600">${q.id}. ${q.text.toLowerCase()}</span>
                <div class="flex gap-2">
                    <button class="btn-oui px-3 py-1 rounded-[8px] text-[11px] border border-[#e5e5e5] hover:bg-[#8cc63f]/10 hover:border-[#8cc63f] transition-all" data-id="${q.id}">oui</button>
                    <button class="btn-non px-3 py-1 rounded-[8px] text-[11px] border border-[#e5e5e5] hover:bg-slate-100 transition-all" data-id="${q.id}">non</button>
                </div>
            `;
            ...
            const handleAnswer = (val) => {
                ...
                if (val === 'oui') {
                    btnOui.className = 'btn-oui px-3 py-1 rounded-[8px] text-[11px] font-bold bg-[#8cc63f] text-white border-[#8cc63f] transition-all';
                    btnNon.className = 'btn-non px-3 py-1 rounded-[8px] text-[11px] border border-[#e5e5e5] bg-white transition-all';
                } else {
                    btnNon.className = 'btn-non px-3 py-1 rounded-[8px] text-[11px] font-bold bg-slate-800 text-white border-slate-800 transition-all';
                    btnOui.className = 'btn-oui px-3 py-1 rounded-[8px] text-[11px] border border-[#e5e5e5] bg-white transition-all';
                }
    ...
    function showSuggestions(suggestions, answers) {
        ...
        wrap.innerHTML = `<span class="text-[10px] text-slate-400 uppercase tracking-wide">suggestions</span>`;

        suggestions.forEach(s => {
            const answer = answers[s.id] || 'non';
            const item = document.createElement('div');
            item.className = `suggestion-item text-[11px] p-2 rounded-[10px] transition-all ${
                answer === 'oui' 
            ...
            item.innerHTML = `
                <div class="flex justify-between items-start gap-4">
                    <span><span class="font-medium text-[10px] opacity-60 mr-1">${s.id}.</span> ${s.text.toLowerCase()}</span>
                    <button class="btn-apply-suggestion shrink-0 text-[10px] px-2 py-0.5 rounded-[6px] border border-slate-200 hover:border-[#8cc63f] hover:bg-[#8cc63f]/10 hover:text-[#8cc63f] transition-all" data-id="${s.id}">appliquer</button>
                </div>
            `;
        ...
        try {
            ...
            if (data.proposed_manifest) {
                if (_refs.applyManifest) {
                    ...
                    btn.className = 'btn-apply-suggestion shrink-0 text-[10px] px-2 py-0.5 rounded-[6px] border border-[#8cc63f] transition-all opacity-50 bg-[#8cc63f]/10 text-[#8cc63f]';
                    btn.disabled = true;
                }
            } else {
